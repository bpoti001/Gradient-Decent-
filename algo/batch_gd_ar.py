...
import random
import sys
my_data = np.genfromtxt('/root/generated_data/generated_data_1.csv', delimiter=',')
row = len(my_data)
...
for a in batch_list:
	...
	while (np.isclose(w,win,atol=5.2e-06).all()==False):
		for  i in range(len(batch_data)):
			...
			err_s = err_s.reshape(coloum,1)
			w = w - alpha_i *err_s
		iteration = iteration+1
	
	f = open('/root/generated_data/output/data_10000_mat/output_batch_i_m_%s.txt' %(str(a)), 'w')
